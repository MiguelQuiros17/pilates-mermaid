...
class AuthService {
  ...
  static verifyToken(token, allowExpired = false) {
    try {
      return jwt.verify(token, JWT_SECRET)
    } catch (error) {
      // If token is expired but we allow expired tokens, decode it without verification
      // This allows us to regenerate expired tokens instead of rejecting them
      if (error.name === 'TokenExpiredError' && allowExpired) {
        try {
          // Decode without verification to get the payload
          const decoded = jwt.decode(token)
          if (decoded && decoded.id && decoded.email && decoded.role) {
            console.log('Token expired but valid, will regenerate:', decoded.id)
            return decoded
          }
        } catch (decodeError) {
      ...
      // Log specific error for expired tokens to help with debugging
...
const requireAuth = async (req, res, next) => {
  try {
    ...
    // First try to verify token normally
    let decoded = AuthService.verifyToken(token)
    let tokenWasExpired = false
    
    // If verification failed, try allowing expired tokens (for old tokens with expiration dates)
    if (!decoded) {
      decoded = AuthService.verifyToken(token, true) // allowExpired = true
      if (decoded) {
        tokenWasExpired = true
        console.log('Token was expired but valid, regenerating for user:', decoded.id)
      }
    }
    ...
    // If token was expired, regenerate it and store in request for endpoints to use
    if (tokenWasExpired) {
      const newToken = AuthService.generateToken(user)
      // Store new token in request so endpoints can include it in response
      req.regeneratedToken = newToken
    }
