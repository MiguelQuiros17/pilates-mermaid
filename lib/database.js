...
class Database {
  constructor() {
    ...
    console.log(`[Database] SQLITE_DB_PATH env var: ${process.env.SQLITE_DB_PATH || 'NOT SET (using default)'}`)
    console.log(`[Database] Current working directory: ${process.cwd()}`)
    
    if (!dbExists) {
      console.warn(`[Database] ⚠️  WARNING: Database file does not exist yet. Will be created on first write.`)
    } else {
      const stats = fs.statSync(DB_PATH)
      console.log(`[Database] Database file size: ${stats.size} bytes`)
      console.log(`[Database] Database file modified: ${stats.mtime.toISOString()}`)
    }
    
    this.db = new sqlite3.Database(DB_PATH)
    console.log(`[Database] Initializing database at: ${DB_PATH}`)
    this.initTables().then(async () => {
      // Log class count after initialization to verify persistence
      try {
        const allClasses = await this.all('SELECT id, is_recurring FROM classes')
        const totalCount = allClasses.length
        const recurringCount = allClasses.filter(c => c.is_recurring === 1 || c.is_recurring === '1' || c.is_recurring === true).length
        const nonRecurringCount = totalCount - recurringCount
        console.log(`[Database] Initialized. Total classes in DB: ${totalCount} (${recurringCount} recurring, ${nonRecurringCount} non-recurring)`)
        if (totalCount === 0) {
          console.warn(`[Database] ⚠️  WARNING: No classes found in database!`)
          console.warn(`[Database] ⚠️  This might indicate:`)
          console.warn(`[Database] ⚠️  1. Database path issue (check SQLITE_DB_PATH env var)`)
          console.warn(`[Database] ⚠️  2. Volume not mounted correctly in Railway`)
          console.warn(`[Database] ⚠️  3. Database was created in non-persistent location`)
        }
      } catch (err) {
        console.log(`[Database] Could not count classes on init:`, err.message)
      }
    }).catch(err => {
  ...
  async run(sql, params = []) {
    // Log all DELETE operations on classes table with full context
    const sqlUpper = sql.trim().toUpperCase()
    if (sqlUpper.includes('DELETE') && sqlUpper.includes('FROM') && sqlUpper.includes('CLASSES')) {
      const stackTrace = new Error().stack
      console.error(`[DATABASE DELETE CLASSES] 🚨 DELETING FROM CLASSES TABLE 🚨`)
      console.error(`[DATABASE DELETE CLASSES] SQL: ${sql}`)
      console.error(`[DATABASE DELETE CLASSES] Params:`, params)
      console.error(`[DATABASE DELETE CLASSES] Stack trace:`)
      console.error(stackTrace)
      
      // Try to get count before deletion for logging
      try {
        if (sqlUpper.includes('WHERE')) {
          // Extract WHERE clause to count affected rows
          const whereIndex = sqlUpper.indexOf('WHERE')
          const whereClause = sql.substring(sqlUpper.indexOf('WHERE'))
          const countSql = `SELECT COUNT(*) as count FROM classes ${whereClause}`
          const countResult = await this.get(countSql, params)
          console.error(`[DATABASE DELETE CLASSES] ⚠️  About to delete ${countResult?.count || 0} class(es)`)
        } else {
          // DELETE FROM classes without WHERE - count all
          const countResult = await this.get('SELECT COUNT(*) as count FROM classes')
          console.error(`[DATABASE DELETE CLASSES] ⚠️  About to delete ALL ${countResult?.count || 0} classes!`)
        }
      } catch (countErr) {
        console.error(`[DATABASE DELETE CLASSES] Could not count classes before deletion:`, countErr.message)
      }
    }
    ...
    // Log after deletion if it was a classes DELETE
    if (sqlUpper.includes('DELETE') && sqlUpper.includes('FROM') && sqlUpper.includes('CLASSES')) {
      console.error(`[DATABASE DELETE CLASSES] ✅ DELETE operation completed`)
    }
  ...
  async deductClassFromUser(userId, classType = 'group', allowOverdraft = false) {
    const user = await this.getUserById(userId)
    if (!user) {
    ...
    if (classType === 'private') {
      const currentRemaining = user.private_classes_remaining || 0
      const newRemaining = currentRemaining - 1
      
      // Check overdraft limit (-2 is maximum)
      if (!allowOverdraft && currentRemaining <= 0) {
        throw new Error('Usuario no tiene clases privadas disponibles')
      }
      if (newRemaining < -2) {
        throw new Error('Has alcanzado el límite máximo de sobregiro (-2 clases). Por favor, cancela una clase o contacta al propietario/instructor para resolver el problema.')
      }
      ...
      const currentRemaining = user.group_classes_remaining || 0
      const newRemaining = currentRemaining - 1
  ...
  async ensureUserPackagesUpToDate(userId, autoRenewDefaultBehavior = null) {
    ...
    if (groupPackage && groupPackage.end_date) {
      let endDate = new Date(groupPackage.end_date)
      endDate.setHours(0, 0, 0, 0)
      
      // If expired, process auto-renewals; handle multiple months overdue
      if (endDate < today) {
        ...
        if (groupPackage.auto_renew && groupPackage.renewal_months && groupPackage.renewal_months > 0) {
          let remainingMonths = groupPackage.renewal_months
          let newEndDate = endDate

          // Process as many months as needed to catch up
          while (remainingMonths > 0 && newEndDate < today) {
            remainingMonths -= 1
          ...
          await this.run(`
            ...
            WHERE id = ?
          `, [newEndDate.toISOString().split('T')[0], todayStr, remainingMonths, groupPackage.id])
          
          // If renewal_months reached 0, mark as expired
          if (remainingMonths === 0 && newEndDate < today) {
            await this.run(`
              UPDATE package_history 
    ...
    // NOTE: We no longer reset class counts to 0 when there's no active package.
    // This allows admins to manually set class counts for users without packages.
    // If no active group package exists, we simply leave the count as-is.
    ...
    if (privatePackage && privatePackage.end_date) {
      let endDate = new Date(privatePackage.end_date)
      endDate.setHours(0, 0, 0, 0)
      ...
      if (endDate < today) {
        ...
        if (privatePackage.auto_renew && privatePackage.renewal_months && privatePackage.renewal_months > 0) {
          let remainingMonths = privatePackage.renewal_months
          let newEndDate = endDate

          // Process multiple overdue months
          while (remainingMonths > 0 && newEndDate < today) {
            remainingMonths -= 1
            newEndDate = new Date(newEndDate)
            newEndDate.setDate(newEndDate.getDate() + 30)
          ...
          await this.run(`
            ...
            SET end_date = ?, last_renewal_date = ?, renewal_months = ?, updated_at = datetime('now')
            WHERE id = ?
          `, [newEndDate.toISOString().split('T')[0], todayStr, remainingMonths, privatePackage.id])
    ...
    // NOTE: We no longer reset class counts to 0 when there's no active package.
    // This allows admins to manually set class counts for users without packages.
    // If no active private package exists, we simply leave the count as-is.
  }
