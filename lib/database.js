...
class Database {
  ...
  async initTables() {
    ...
    // Add bilingual columns for classes
    try {
      await run(`ALTER TABLE classes ADD COLUMN title_es TEXT`)
    } catch (e) {}
    try {
      await run(`ALTER TABLE classes ADD COLUMN title_en TEXT`)
    } catch (e) {}
    try {
      await run(`ALTER TABLE classes ADD COLUMN description_es TEXT`)
    } catch (e) {}
    try {
      await run(`ALTER TABLE classes ADD COLUMN description_en TEXT`)
    } catch (e) {}
    
    // Migrate existing title and description to both languages
    try {
      const all = promisify(this.db.all.bind(this.db))
      const classes = await all(`SELECT id, title, description FROM classes WHERE title_es IS NULL OR title_en IS NULL`)
      for (const cls of classes) {
        if (cls.title) {
          await run(`UPDATE classes SET title_es = ?, title_en = ? WHERE id = ?`, [cls.title, cls.title, cls.id])
        }
        if (cls.description) {
          await run(`UPDATE classes SET description_es = ?, description_en = ? WHERE id = ?`, [cls.description, cls.description, cls.id])
        }
      }
    } catch (e) {
      console.error('Error migrating class bilingual fields:', e)
    }
    ...
    // Add bilingual columns for packages
    try {
      await run(`ALTER TABLE packages ADD COLUMN name_es TEXT`)
    } catch (e) {}
    try {
      await run(`ALTER TABLE packages ADD COLUMN name_en TEXT`)
    } catch (e) {}
    try {
      await run(`ALTER TABLE packages ADD COLUMN description_es TEXT`)
    } catch (e) {}
    try {
      await run(`ALTER TABLE packages ADD COLUMN description_en TEXT`)
    } catch (e) {}
    
    // Migrate existing name and description to both languages
    try {
      const all = promisify(this.db.all.bind(this.db))
      const packages = await all(`SELECT id, name, description FROM packages WHERE name_es IS NULL OR name_en IS NULL`)
      for (const pkg of packages) {
        if (pkg.name) {
          await run(`UPDATE packages SET name_es = ?, name_en = ? WHERE id = ?`, [pkg.name, pkg.name, pkg.id])
        }
        if (pkg.description) {
          await run(`UPDATE packages SET description_es = ?, description_en = ? WHERE id = ?`, [pkg.description, pkg.description, pkg.id])
        }
      }
    } catch (e) {
      console.error('Error migrating package bilingual fields:', e)
    }
    ...
    // Add bilingual columns for payments
    try {
      await run(`ALTER TABLE payments ADD COLUMN concept_es TEXT`)
    } catch (e) {}
    try {
      await run(`ALTER TABLE payments ADD COLUMN concept_en TEXT`)
    } catch (e) {}
    
    // Migrate existing concept to both languages
    try {
      const all = promisify(this.db.all.bind(this.db))
      const payments = await all(`SELECT id, concept FROM payments WHERE concept_es IS NULL OR concept_en IS NULL`)
      for (const payment of payments) {
        if (payment.concept) {
          await run(`UPDATE payments SET concept_es = ?, concept_en = ? WHERE id = ?`, [payment.concept, payment.concept, payment.id])
        }
      }
    } catch (e) {
      console.error('Error migrating payment bilingual fields:', e)
    }
  ...
  async createPackage(packageData) {
    ...
    await this.run(`
      INSERT INTO packages (
        id, name, name_es, name_en, type, classes_included, price, validity_days, validity_months, category, description, description_es, description_en, is_live, live_from, live_until, original_price, sale_price, is_active
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      id, 
      packageData.name || packageData.name_es || packageData.name_en, // Keep name for backward compatibility
      packageData.name_es || packageData.name_en || packageData.name || '',
      packageData.name_en || packageData.name_es || packageData.name || '',
      packageData.type, 
      ...
      packageData.description || packageData.description_es || packageData.description_en || '', // Keep description for backward compatibility
      packageData.description_es || packageData.description_en || packageData.description || '',
      packageData.description_en || packageData.description_es || packageData.description || '',
