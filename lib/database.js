    ...
    if (groupPackage && groupPackage.end_date) {
      ...
      if (endDate < today) {
        ...
        if (groupPackage.auto_renew) {
          ...
          await this.run(`
            UPDATE package_history 
            SET end_date = ?, last_renewal_date = ?, updated_at = datetime('now')
            WHERE id = ?
          `, [newEndDate.toISOString().split('T')[0], todayStr, groupPackage.id])
      ...
      if (endDate < today) {
        ...
        if (privatePackage.auto_renew) {
          ...
          await this.run(`
            ...
            SET end_date = ?, last_renewal_date = ?, updated_at = datetime('now')
            WHERE id = ?
          `, [newEndDate.toISOString().split('T')[0], todayStr, privatePackage.id])
