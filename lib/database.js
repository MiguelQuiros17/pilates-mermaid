...
class Database {
  ...
  async createBooking(booking) {
    ...
    await this.run(`
      INSERT INTO bookings (id, user_id, class_id, booking_date, status, payment_method, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [
  ...
  async getBookingsByUser(userId) {
    return await this.all(`
      SELECT b.*, c.title, c.date, c.time, c.type, c.coach_name
      FROM bookings b
  ...
  async checkUserAvailableClasses(userId) {
    const user = await this.getUserById(userId)
    if (!user) return { hasClasses: false, remaining: 0 }
    
    return {
      hasClasses: user.clases_restantes > 0,
      remaining: user.clases_restantes || 0,
      packageType: user.type_of_class
    }
  }

  // Helper method to deduct class from user's package
  async deductClassFromUser(userId) {
    const user = await this.getUserById(userId)
    if (!user || user.clases_restantes <= 0) {
      throw new Error('Usuario no tiene clases disponibles')
    }
    
    const newRemaining = user.clases_restantes - 1
    const newTaken = (user.clases_tomadas || 0) + 1
    
    await this.run(`
      UPDATE users 
      SET clases_restantes = ?, clases_tomadas = ?, updated_at = datetime('now')
      WHERE id = ?
    `, [newRemaining, newTaken, userId])
