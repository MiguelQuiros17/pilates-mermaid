...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  useEffect(() => {
    const verifyAuth = async () => {
      ...
      // The backend will automatically regenerate expired tokens
      // We only need to check if token exists
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.user) {
            ...
            // If a new token was provided (automatic refresh for expired tokens), update it
            if (data.token) {
              localStorage.setItem('token', data.token)
              console.log('Token automatically regenerated (was expired)')
            }
          } else {
          ...
          // Check if it's a 401 (unauthorized) - only redirect if it's truly invalid
          // Note: Expired tokens should be regenerated by the backend, so 401 here means invalid token
          if (response.status === 401) {
            const errorData = await response.json().catch(() => ({}))
            // Only redirect if the error message indicates the token is truly invalid, not just expired
            // If it says "Token inválido" (invalid) vs "Token inválido o expirado" (expired), 
            // expired tokens should have been handled by the backend
            ...
            // For expired tokens, the backend should regenerate them, so this shouldn't happen
            // But if it does, try one more time
            console.warn('Unexpected 401, token may have been expired but not regenerated')
          }
          // For other errors, log but don't immediately redirect - might be temporary
          console.error('Authentication verification failed:', response.status)
        }
      } catch (error) {
        console.error('Error verifying authentication:', error)
        // On network error or other issues, clear auth and redirect to login
        // This ensures users with expired tokens are always redirected
        localStorage.removeItem('token')
        localStorage.removeItem('user')
        window.location.href = '/login'
        return
